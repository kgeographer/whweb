...
$sql = "select el_id, title, year, countries, description, nphrases, array_to_string(array(select distinct unnest(string_to_array(verbs,', '))),', ') AS verbs, region, ST_AsGeoJSON(st_centroid(iregions.geom)) AS centroid from elements left join iregions on elements.region=iregions.objectid where region in ($regions) order by el_id";
...
$rs = $conn->query($sql);
if (!$rs) {
    echo 'An SQL error occurred. You sent '.$sql;
    exit;
}
